# Scenario 1
# Changes in the content of information
# 
...
for (simul in 1:100){
  ...
  s1_record <- data.frame(scenario = rep(1:4, each = 2),
                          info = rep(1,8),
                          time = rep(1:2, 4), 
                          sourcenode = rep(sample(1:N_nodes,2, replace = FALSE),4),
                          posneg = c(0,0,0,1,1,0,1,1),
                          original = rep(1,8)) # True --> put back
  s1_record$from <- s1_record$sourcenode
  ...
  for (sce in 1:4) {
    ...
    s_record <-  subset(s1_record, scenario == sce, select = c("info","time","sourcenode","posneg","original","from"))
    ...
    dec_s1_table <- rbind(dec_s1_table,dec_table) # decision | >= belief bound
    vac_s1_summary <- rbind(vac_s1_summary,vac_summary)  # status | accept, reject
    
    cat(paste0('scenario:',sce,'\n'))
  }
  
  dec_table_simul <- rbind(dec_table_simul, dec_s1_table)
  vac_summary_simul <- rbind(vac_summary_simul, vac_s1_summary)
  
  cat(paste0('simulation:',simul,'\n'))
  
}

saveRDS(vac_summary_simul,'s1_vac_status.rds')
saveRDS(dec_table_simul,'s1_estimated_vaxer.rds')
