# Scenario 4
# Difference in weights of the sub-criteria
# 
...
for (simul in 1:100){
  ...
  w_i_part <- rbind(t(rdirichlet(N_nodes, rep(1,3)))) 
  ...
  scenario_s3 <- matrix(0.5, nrow = 2, ncol = N_nodes) %>% as.data.frame()
  
  scenario_s4 <- matrix(0, nrow = 2, ncol = N_nodes) %>% as.data.frame()
  scenario_s4[1,] <- scenario_s1[2,]
  scenario_s4[2,] <- scenario_s1[1,]
  
  scenario_s5 <- matrix(0, nrow = 2, ncol = N_nodes) %>% as.data.frame()
  scenario_s5[1,] <- scenario_s2[2,]
  scenario_s5[2,] <- scenario_s2[1,]

  s_w_whole <- matrix(0, nrow = 25, ncol = N_nodes + 1) %>% as.data.frame()
  names(s_w_whole)[1] <- 'scenario'
  s_w_whole$scenario <- rep(1:5, each = 5)
  s_w_whole[1:2,2:(N_nodes+1)] <- scenario_s1
  s_w_whole[6:7,2:(N_nodes+1)] <- scenario_s2
  s_w_whole[11:12,2:(N_nodes+1)] <- scenario_s3
  s_w_whole[16:17,2:(N_nodes+1)] <- scenario_s4
  s_w_whole[21:22,2:(N_nodes+1)] <- scenario_s5
  for (q in 1:5) {
    s_w_l <- 3+5*(q-1)
    s_w_u <- 5+5*(q-1)
    s_w_whole[s_w_l:s_w_u,2:(N_nodes+1)] <- w_i_part
  }
  ...
  s4_record <- data.frame(scenario = rep(1:5,each = 4),
                          info = c(rep(1:4,3),rep(c(3,4,3,4),2)),
                          time = rep(1:4,5), 
                          sourcenode = rep(sample(1:N_nodes,4,replace = FALSE),5),
                          posneg = rep(1,20),
                          original = rep(1,20)) # True --> put back
  s4_record$from <- s4_record$sourcenode
  ...
  for (sce in 1:5) {
    ...
    w_i_whole <- s_w_whole[s_w_whole$scenario == sce,2:(1 + N_nodes)]
    ...
    s_record <-  subset(s4_record, scenario == sce, select = c("info","time","sourcenode","posneg","original","from"))
    ...
    dec_s4_table <- rbind(dec_s4_table,dec_table) # decision | >= belief bound
    vac_s4_summary <- rbind(vac_s4_summary,vac_summary)  # status | accept, reject
    
    cat(paste0('scenario:',sce,'\n'))
  }
  
  dec_table_simul <- rbind(dec_table_simul, dec_s4_table)
  vac_summary_simul <- rbind(vac_summary_simul, vac_s4_summary)
  
  cat(paste0('simulation:',simul,'\n'))
  
}

saveRDS(vac_summary_simul,'s4_vac_status.rds')
saveRDS(dec_table_simul,'s4_estimated_vaxer.rds')
